...
/**
 * Adds baseline HTTP security response headers to every response served by users-service
 ...
 *       body from ever being reinterpreted as HTML/script (defense-in-depth for the stored member
 *       {@code name} noted as non-exploitable INFO in the QA report, should a render sink ever be
 *       added).</li>
 ...
 *       response (CWE-525). Critical here because {@code /api/members} returns member PII
 *       ({@code name}/{@code email}/{@code phoneNumber}); no-store keeps that data out of
 *       intermediary and browser caches.</li>
 * </ul>
 *
 ...
 * responses (e.g. {@code /api/members}, {@code /api/members/{id}/tier}), on validation
 * {@code 400}/{@code 409} responses, and even on the {@code InternalApiKeyFilter}'s
...
public class SecurityHeadersFilter extends OncePerRequestFilter {
    ...
    /** Forbids caching of responses (protects member PII payloads from intermediaries). */
