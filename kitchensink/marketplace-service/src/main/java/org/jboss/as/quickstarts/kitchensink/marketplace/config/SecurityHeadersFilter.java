package org.jboss.as.quickstarts.kitchensink.marketplace.config;
 ...
 * marketplace-service (REST API, actuator, and error dispatches alike).
 *
 ...
 * </ul>
 * <p>{@code Cache-Control} is intentionally NOT forced here: the public product catalog is safely
 * cacheable and globally pinning {@code no-store} would be a behavioural change beyond this
 * hardening fix. {@code Content-Security-Policy}/{@code Permissions-Policy} are document/browser
 * feature-context headers with no meaningful effect on non-rendered JSON, so they are omitted to
 * keep the change minimal and free of per-consumer tuning. Each header is only set when absent, so
 * a handler that intentionally sets its own value is never clobbered.</p>
 */
@Component
