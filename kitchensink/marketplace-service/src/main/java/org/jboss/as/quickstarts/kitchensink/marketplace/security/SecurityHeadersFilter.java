package org.jboss.as.quickstarts.kitchensink.marketplace.security;
...
/**
 * Adds baseline HTTP security response headers to every response served by marketplace-service
 ...
 *       response (CWE-525). Applied uniformly so data endpoints (and, in the other two services,
 *       member PII / order / spend payloads) are never retained by an intermediary.</li>
 * </ul>
 *
 ...
 * before any other filter or handler can commit the response — guaranteeing their presence on
 * successful {@code 2xx} responses as well as on validation {@code 400}s and any error responses.
 * The headers are set on the {@link HttpServletResponse} prior to invoking the chain, so they
 * persist regardless of the downstream outcome.</p>
 *
 ...
 * preserving the intentional no-auth model of the public {@code /api/**} surface. The class is
...
public class SecurityHeadersFilter extends OncePerRequestFilter {
    ...
    /** Forbids caching of responses (protects PII / order / spend payloads from intermediaries). */
