...
<?php if ($success): ?>
    <div class="alert alert-success" data-testid="checkout-success">
        <?php echo htmlspecialchars($success); ?>
        <br><a href="/frontend/index.php">Return to home</a>
    </div>
<?php elseif ($error): ?>
    <div class="alert alert-error" data-testid="checkout-error"><?php echo htmlspecialchars($error); ?></div>
<?php endif; ?>

<?php if (!$orderId): ?>

<?php if ($preview && !empty($preview['items'])): ?>
<div class="card" style="margin-bottom:24px;" data-testid="order-summary">
    ...
    <table style="box-shadow:none; background:transparent; margin-bottom:16px;">
        ...
        <?php endforeach; ?>
        <tr style="border-top:1px solid #ddd;"><td>Subtotal</td><td class="price" data-testid="checkout-subtotal"><?php echo format_currency($preview['subtotal']); ?></td></tr>
        <tr><td>Discount (<?php echo htmlspecialchars($_SESSION['member_tier'] ?? ''); ?> tier)</td><td class="price" style="color:#27ae60;" data-testid="checkout-discount">- <?php echo format_currency($preview['discountAmount']); ?></td></tr>
        <tr><td>Shipping to <?php echo htmlspecialchars($zip); ?><?php echo $expedite ? ' (Expedited)' : ''; ?></td><td class="price" data-testid="checkout-shipping"><?php echo format_currency($preview['shippingCost']); ?></td></tr>
        <tr style="border-top:2px solid #1a3a5c;">
            <td><strong>Total</strong></td>
            <td class="price" style="font-size:1.1rem;" data-testid="checkout-total"><strong><?php echo format_currency($preview['total']); ?></strong></td>
        </tr>
    </table>

    <form method="POST">
        <input type="hidden" name="zip"      value="<?php echo htmlspecialchars($zip); ?>" data-testid="checkout-zip">
        <input type="hidden" name="expedite" value="<?php echo $expedite ? '1' : '0'; ?>" data-testid="checkout-expedite">
        <button type="submit" name="confirm_order" class="btn btn-success" style="font-size:1rem; padding:12px 28px;" data-testid="checkout-confirm">
            Confirm &amp; Place Order
        </button>
        <a href="/frontend/cart.php" class="btn" style="margin-left:12px; background:#e0e7ef; color:#1a3a5c;" data-testid="checkout-back">Back to Cart</a>
    </form>
</div>
<?php elseif ($zip): ?>
    <div class="alert alert-error" data-testid="checkout-error">Could not load order preview. Your cart may be empty.</div>
    <a href="/frontend/cart.php" class="btn btn-primary">Back to Cart</a>
<?php else: ?>
    <div class="alert alert-error" data-testid="checkout-error">No ZIP code provided.</div>
    <a href="/frontend/cart.php" class="btn btn-primary">Back to Cart</a>
<?php endif; ?>
